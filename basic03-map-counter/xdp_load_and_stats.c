			...
			struct stats_record *stats_prev)
{
	...
	for (int i = 0; i < XDP_ACTION_MAX; i++) {
		...
		__u64 totalReceivedKBytes = rec->total.rx_bytes / 1000;

		__u64 newBytes = rec->total.rx_bytes - prev->total.rx_bytes;
		__u64 newBits = newBytes * 8;
		double newBitsPerSecond = newBits / period;
		double newMBitsPerSecond = newBitsPerSecond / 1000000;

		printf(
			fmt,
			action, rec->total.rx_packets, pps,
			totalReceivedKBytes, newMBitsPerSecond,
			period
		);
	}
}
